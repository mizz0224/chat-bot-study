...
function response(room, msg, sender, isGroupChat, replier, ImageDB, packageName, threadId) {
    try {
        if (room == MANAGER) {
            if (msg == "ㅎㅇ") {
                replier.reply(call_Privacy_By_File(sender));
            } else if (msg == "ㅂㅇ") {
                replier.reply("일단 실행은함 ㅇㅇ");
                save_Privacy_By_File(sender, "성공적으로 저장");
            }
        } else {
...
function save_Privacy_By_File(sender, privacy) {
    try {
        privacy_str = return_Array_By_String(privacy, "\n");
        var folder = new java.io.File(folder_Root);
        folder.mkdirs();
        var file = new java.io.File(folder_Root + sender + TEXTFORM);
        var fos = new java.io.FileOutputStream(file);
        contents = new java.lang.String(privacy);
